...
namespace Game
{
    public class MyCollectionPartTimeJobListPanel : BaseCustomPanel
    {
        private IScrollView mScrollView;
        private int mLastID;
        private bool mIsClear;
        public MyCollectionPartTimeJobListPanel()
        {
        ...
        private void GetMyCollectionPartTimeJob()
        {
            byte[] sendBytes = ByteTools.Concat(AppVarData.Account.ToBytes(), mLastID.ToBytes());
            AppTools.TcpSend(TcpSubServerType.Login, (short)TcpLoginUdpCode.GetMyCollectionPartTimeJobList, sendBytes);
        }
